...
const SignIn = () => {
  ...
  return (
    ...
    {loged===null?<div className='flex flex-col justify-center items-center py-[9%]'>
        ...
        {status===undefined?'':<div className='fixed bg-stone-900 bg-opacity-90 top-0 left-0 w-full h-screen z-40 flex justify-center items-center'><div className={`rounded-lg py-2 px-4 font-bold flex justify-center items-center ${status===true?'text-stone-50 bg-green-200 shadow-md block absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[22rem] h-[22rem] z-50':status===false?'text-red-50 bg-red-100 shadow-md block absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[22rem] h-[22rem] z-50':''}`}>{status===true?<div><span className='relative flex flex-col gap-8 text-stone-800 font-bold'><img className='w-40 h-40' src={done} alt={done}/><span className='text-center'>Success!</span></span><Link to="/create"><XMarkIcon onClick={()=>setStatus(undefined)} className='absolute top-2.5 right-2 text-stone-800 w-9 h-9 cursor-pointer rounded-full bg-stone-300 p-1'/></Link></div>:status===false?<div><span className='relative flex flex-col gap-8 text-stone-800 font-bold justify-center'><img className='w-20 h-20' src={cancel} alt={cancel}/><span className='text-center'>Failed!</span></span><XMarkIcon onClick={()=>setStatus(undefined)} className='absolute top-2.5 right-2 text-stone-800 w-9 h-9 cursor-pointer rounded-full bg-stone-300 p-1'/></div>:''}</div></div>}
        <form onSubmit={(event)=>submit(event)} className='px-9 flex flex-col gap-5'>
            <div className='relative'>
              ...
              <UserIcon className='absolute text-stone-50 w-6 top-0 left-0 p-1 h-[34px] bg-[#39D0FF]'/>
            </div>
            <div className='relative flex-col gap-1'>
              ...
              <UserPlusIcon className='absolute text-stone-50 w-6 top-0 left-0 p-1 h-[34px] bg-[#39D0FF]'/>
            </div>
            <div className='relative'>
              ...
              <EnvelopeIcon className='absolute text-stone-50 w-6 top-0 left-0 p-1 h-[34px] bg-[#39D0FF]'/>
            </div>
            <div className='relative w-full '>
              ...
              <DevicePhoneMobileIcon className='absolute text-stone-50 w-6 top-0 left-0 p-1 h-[34px] bg-[#39D0FF]'/>
            </div>
