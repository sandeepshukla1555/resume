...
const Header = () => {
    const {status,changeStatus}=useContext(Context)
    ...
    useEffect(()=>{
      ...
      {
        const toObject=JSON.parse(status)
       ...
       <div className='flex justify-between gap-1 items-center absolute top-3 md:top-5 w-full px-2 md:px-5'>
        ...
        <div className='flex justify-between items-center w-[95%] md:w-[60%]'>
            <ul className='flex md:justify-between gap-2 overflow-scroll flex-nowrap w-full md:w-[70%] scrollHeight'>
              <li><Link to='/' className='text-sm md:text-lg text-stone-50 cursor-pointer'>Home</Link></li>
              <li><Link to='/update' className='text-sm md:text-lg text-stone-50 border-l md:border-l-0 pl-0.5 cursor-pointer'>Update</Link></li>
              <li><Link to='/download' className='text-sm md:text-lg text-stone-50 border-l md:border-l-0 pl-0.5 cursor-pointer'>Download</Link></li>
              {status===null?<li><Link to='/signin' className='text-sm md:text-lg text-stone-50 border-l md:border-l-0 pl-0.5 cursor-pointer'>Signin</Link></li>:<li><span onClick={()=>changeStatus()} className='text-sm md:text-lg text-stone-50 border-l md:border-l-0 pl-0.5 cursor-pointer'>Logout</span></li>}
            </ul>
              {
                resumes===null?<div className='flex flex-col gap-1 items-center w-[10%]'><div className='w-9 h-9 rounded-full bg-stone-800 flex justify-center items-center text-xs text-stone-50 uppercase'></div><span className='text-[5px] text-stone-800 text-center'>Not Sign-In</span></div>:<div className='flex flex-col gap-1 items-center w-[10%]'><div className='w-9 h-9 rounded-full bg-stone-800 flex justify-center items-center text-xs text-stone-50 uppercase'>{status===null?'':`${resumes.fName[0]+"."+resumes.lName[0]+"."}`}</div><span className='text-[5px] text-stone-800 text-center'>{resumes.fName+" "+resumes.lName}</span></div>
              }
        </div>
    </div>
    </div>
